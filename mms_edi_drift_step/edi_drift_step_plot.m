...
% 		set (hBPP_figure, 'DockControls', 'off')
	...
	zeroVector = [ 0.0 ];
	hBPP_plotElements (6) = plot3 (S_star_bpp (1), S_star_bpp (2), zeroVector (1), ...
...
% 		S_star_bpp, GrubbsBeamInterceptSD, GrubbsBeamInterceptSDOM) )

	ConfidenceIntervals = [ ...
		S_star_bpp-d_bpp_CI, ... % x|y lower limits
		S_star_bpp+d_bpp_CI ];   % x|y upper limits
	...
	if show_unit_vectors
		...
		S_star_bpp_u = S_star_bpp / norm (S_star_bpp, 2);
...
% 		size (GrubbsBeamIntercepts, 2)
	...
	title ( { ...
		...
		[ 'MMS', obsID, ': ', bTimeStr, ', P_{0} = ', num2str(P0, 3), ', Points = ', num2str(nBeamIntercepts), ', Q = ', num2str(d_quality) ];
		[ 'B_{DMPA} [', ...
		...
		[ 'd CI bpp [', ...
		  num2str(d_bpp_CI (1),'%5.2f'), ',', ...
		  num2str(d_bpp_CI (2),'%5.2f'), ']' ...
		]; ...
