...
if ~isequal (mms_ql__EDI__B__dataFile, 0) % then a valid [we hope] file selected
	...
	edi_E_dmpa = spdfcdfread (mms_ql__EDI__B__data, ...
		'CombineRecords',        true, ...
...
% keyboard
		...
		dceFillVal = CDF_FillVal_single;
		idceFillVal = find (edp_dce_xyz_dsl (:, 1) < dceFillVal);
...
% 		  datestr(spdftt2000todatenum(edp_tt2000(end)), 'yyyy-mm-dd HH:MM:ss') ]
	...
	if ~isempty (iBeqNaN)
		iBeqNaN % debug CDF file for NaNs.
	end
	edi_B_dmpa_FillVal = CDF_FillVal_single;
	...
	edi_gd_ID           = [ zeros(1, size(edi_gd12_fv_dmpa, 1), 'uint8')+1, zeros(1, size(edi_gd21_fv_dmpa, 1), 'uint8')+2 ];
	% ... and tranpose B records
	BdvE_datenum    = spdftt2000todatenum (edi_BdvE_tt2000);
	...
	iEDP_lt_EDI = find (edp_datenum < BdvE_datenum (1));
	iEDP_gt_EDI = find (edp_datenum > BdvE_datenum (end));
