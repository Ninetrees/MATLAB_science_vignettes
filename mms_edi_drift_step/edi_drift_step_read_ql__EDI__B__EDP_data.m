...
% ~~~~~~~~~~~~~~~~~~~
if UseFileOpenGUI
	[mms_ql_EDI_BdvE_dataFile, mms_ql_dataPath] = uigetfile ('mms*.cdf', 'Select an MMS ql EDI_&_B CDF file');
	if isequal (mms_ql_EDI_BdvE_dataFile,  0) % then no valid file selected
		msgbox ('No valid MMS ql EDI_&_B data file selected.');
	else
		mms_ql_EDI_BdvE_data = [mms_ql_dataPath, mms_ql_EDI_BdvE_dataFile];
	end
end

obsID = mms_ql_EDI_BdvE_dataFile (4:4);
YYYY  = str2num (mms_ql_EDI_BdvE_dataFile (30:33));
MM    = str2num (mms_ql_EDI_BdvE_dataFile (34:35));
DD    = str2num (mms_ql_EDI_BdvE_dataFile (36:37));
...
if ~isequal (mms_ql_EDI_BdvE_dataFile, 0) % then a valid [we hope] file selected
	disp ([ 'Reading MMS EDI_&_B data... ', mms_ql_EDI_BdvE_data ])
	mms_ql_EDI_dataFile_info = spdfcdfinfo (mms_ql_EDI_BdvE_data);
	...
	% ~~~~~~~~~~~~~~~~~~~ B d v E
	edi_BdvE_t2k = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              'Epoch', ...
		'ConvertEpochToDatenum', false, ...
		'KeepEpochAsIs',         true);
	edi_B_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_B_dmpa']);
	edi_B_std_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_B_std_dmpa']);
	edi_E_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_E_dmpa']);
	edi_E_sdcs = edi_E_sdcs';

	% The record number of edi_B_sdcs to which 5s sets of EDI beams map.
	edi_BdvE_recnum = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
	...
	% ~~~~~~~~~~~~~~~~~~~ GDU data
	edi_gd12_beam_t2k = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              'epoch_gd12_beam', ...
		'ConvertEpochToDatenum', false, ...
		'KeepEpochAsIs',         true);
	edi_gd12_virtual_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_pos_virtual_gun1_dmpa']);
	edi_gd12_fv_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_fv_gd12_dmpa']);
	edi_gd12_xref2_B = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_recnum_gd12']);
% 	edi_gd12_quality = spdfcdfread (mms_ql_EDI_BdvE_data, ...
% 		'CombineRecords',        true, ...
% 		'Variable',              ['mms', obsID, '_edi_beam_quality_gd12']);
	...
	% ~~~~~~~~~~~~~~~~~~~ GDU data
	edi_gd21_beam_t2k = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              'epoch_gd21_beam', ...
		'ConvertEpochToDatenum', false, ...
		'KeepEpochAsIs',         true);
	edi_gd21_virtual_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_pos_virtual_gun2_dmpa']);
	edi_gd21_fv_sdcs = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_fv_gd21_dmpa']);
	edi_gd21_xref2_B = spdfcdfread (mms_ql_EDI_BdvE_data, ...
		'CombineRecords',        true, ...
		'Variable',              ['mms', obsID, '_edi_recnum_gd21']);
% 	edi_gd21_quality = spdfcdfread (mms_ql_EDI_BdvE_data, ...
% 		'CombineRecords',        true, ...
% 		'Variable',              ['mms', obsID, '_edi_beam_quality_gd21']);
	...
	% ~~~~~~~~~~~~~~~~~~~
	if UseFileOpenGUI
		[mms_ql_EDP_dataFile, mms_ql_dataPath] = uigetfile ('mms*.cdf', 'Select an MMS ql EDP CDF file');
		if isequal (mms_ql_EDP_dataFile,  0) % then no valid file selected
			msgbox ('No valid MMS ql EDP data file selected.');
		else
			mms_ql_EDP_data = [mms_ql_dataPath, mms_ql_EDP_dataFile];
		end
	end

	if ~isequal (mms_ql_EDP_dataFile, 0) % then a valid [we hope] file selected
		disp ([ 'Reading EDP data... ', mms_ql_EDP_data ])
		mms_ql_EDP_dataFile_info = spdfcdfinfo (mms_ql_EDP_data);
		edp_dce_varInfo = CDF_varInfo (mms_ql_EDP_dataFile_info, ['mms', obsID, '_edp_dce_xyz_dsl']);
		edp_dce_fillVal = edp_dce_varInfo.FillVal;
% keyboard
		% ~~~~~~~~~~~~~~~~~~~ DC E-field
		edp_t2k = spdfcdfread (mms_ql_EDP_data, ...
			'CombineRecords',        true, ...
			'Variable',              ['mms', obsID, '_edp_dce_epoch'], ...
			'ConvertEpochToDatenum', false, ...
			'KeepEpochAsIs',         true);
		% Electric field in DSL coordinates (DSL ~= DMPA ~= DBCS)
		edp_dce_xyz_sdcs = spdfcdfread (mms_ql_EDP_data, ...
			'CombineRecords',        true, ...
...
end % ~isequal (mms_ql_EDI_BdvE_dataFile, 0)
